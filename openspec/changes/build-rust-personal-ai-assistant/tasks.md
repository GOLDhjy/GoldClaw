...
- [x] 2.1 设计 `config.toml`、profile 覆盖和环境变量覆写规则
- [x] 2.2 实现非敏感配置加载、schema 校验和默认值填充
- [ ] 2.3 接入 OS keyring 或等价密钥后端，支持凭据存储与读取
- [x] 2.4 设计 SQLite schema，覆盖会话索引、消息元数据、任务队列、连接器状态和诊断缓存
...
- [x] 5.2 实现可复用的检查项框架，支持 fatal、warning、info 三种严重级别
- [ ] 5.3 增加配置、密钥、Provider 连通性、端口占用、数据库迁移和本地目录权限检查
- [ ] 5.4 增加 Webhook、渠道授权和连接器健康检查
- [x] 5.5 实现 `doctor --json` 与人类可读输出格式
...
- [x] 6.3 使用 `ratatui` 构建 TUI 会话列表、消息面板、状态栏和诊断视图
- [x] 6.4 让 TUI 接入统一事件流，支持流式输出、工具事件和错误提示
- [x] 6.5 实现会话切换、历史查看、快捷键帮助和重连逻辑
...
- [x] 9.3 明确微信 v1 支持边界，并实现对应接入路径或实验性适配器骨架
...
- [x] 11.3 设计本地目录结构，包括配置、数据库、日志、缓存和临时文件位置
